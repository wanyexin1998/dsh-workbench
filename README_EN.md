...
<p align="center">
  ...
  <img alt="Version: 0.2.0-rc.5" src="https://img.shields.io/badge/version-0.2.0--rc.5-2563EB">
...
> [!IMPORTANT]
> Version `0.2.0-rc.5` is a source preview, not a one-command npm release. Split Pane requires the pinned Harness fork. Better Sidebar and Panel Compatibility are entirely optional. This project never installs, updates, or modifies a third-party plugin automatically.
...
> ```
>
> Full decision rules and commands live in [`docs/INSTALL.md`](docs/INSTALL.md). The `v0.2.0-rc.5` GitHub Release is published — both the general-plugin and Split Pane bootstrap paths work today. Status still reads `0.2.0-rc.5` source preview (see [`release-contract.json`](release-contract.json)); that is not an npm release. To audit the source yourself, use "Advanced: build from source" below.
...
| DSH Workbench | Yes | `0.2.0-rc.5` | Maximum two visible Panes |
...
| Panel Compatibility | Optional | `0.1.0-rc.3` | Connects only explicit compatible providers |
...
The current public contract allows at most two visible Panes. Five Panes require new layout, capacity, and performance acceptance work and are outside `0.2.0-rc.5`.
</details>
