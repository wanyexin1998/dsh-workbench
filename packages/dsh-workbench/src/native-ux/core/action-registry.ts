...
export interface ActionDef {
   ...
   * entire job IS an explicit chord gesture the user fires from inside an
   * editable element (e.g. workbench.session.new's Primary+N, meant to work
   * even while composing), since the chord firing while typing is the whole
   * point of that gesture, not an accident to suppress. */
