...
- [x] `docs/adr/` 新增两篇 ADR，编号顺延既有序列，风格与既有 ADR 一致
- [x] `ConversationSelection` / `SelectionActions` 接口定型并被 ADR 引用，字段语义（parentSessionId 冻结时机、atSeq 含义）写明
- [x] 参考实现 `AHGGG/dsh-side-chat` 的固定 SHA 与 MIT 许可移植范围记录在案
- [x] 决策内容与 `../design.md` §7 决策记录逐条一致，无新增未裁决项
